...
Output:
  dataset/yolo17_data/
    balanced/{joint,bone,motion}/
      dataset.npz  (x_train, y_train, x_test, y_test)
      label_map.json
      dataset_info.json
...
def save_npz_dataset(out_dir: Path, x_train, y_train, x_test, y_test, npz_name):
    ...
    c_train = Counter(y_train[:, 1].astype(int).tolist())
    c_test = Counter(y_test[:, 1].astype(int).tolist())
    print(f"    {out_path.name}  shape(x_train)={x_train.shape}  "
          f"train_fall={c_train.get(1, 0)} train_not_fall={c_train.get(0, 0)}")
    print(f"    {out_path.name}  shape(x_test)={x_test.shape}  "
          f"test_fall={c_test.get(1, 0)} test_not_fall={c_test.get(0, 0)}")


def make_version(out_dir: Path, train_s, val_s, max_frames, version_name, transform=None, npz_name="dataset.npz"):
    out_dir.mkdir(parents=True, exist_ok=True)
    print(f"\n  [{version_name}]")
    ...
    y_train = np.zeros((len(tl), 2), dtype=np.float32)
    y_test = np.zeros((len(vl), 2), dtype=np.float32)
    for i, lbl in enumerate(tl):
        y_train[i, int(lbl)] = 1.0
    for i, lbl in enumerate(vl):
        y_test[i, int(lbl)] = 1.0

    save_npz_dataset(out_dir, td, y_train, vd, y_test, npz_name)
...
def main():
    ...
    for mod, fn in mods:
        make_version(bal / mod, tr_bal, va_all, args.max_frames,
                     f"balanced/{mod}", transform=fn, npz_name=args.npz_name)
    ...
    for mod, fn in mods:
        make_version(full / mod, tr_all, va_all, args.max_frames,
                     f"full/{mod}", transform=fn, npz_name=args.npz_name)
