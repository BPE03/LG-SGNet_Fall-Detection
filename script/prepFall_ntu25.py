...
SKELETON_PATH = "data/nturgbd_raw/nturgb+d_skeletons"
OUTPUT_PATH  = "data/ntu/fall_detection"
...
def to_tensor(seq: np.ndarray, target: int) -> np.ndarray:
    """(T, 25, 3) → (target, 75)  flat, matching feeder load_data expectation."""
    s = pad_or_crop(seq, target)    # (target, 25, 3)
    return s.reshape(target, -1)    # (target, 75)
...
MIN_VALID_FRAMES = 10  # drop sequences with fewer real frames than this
...
def parse_sample(filepath: str, max_frames: int) -> np.ndarray:
    ...
    # Count valid frames AFTER normalization — check what the feeder will actually see
    real_frames = int(np.any(seq.reshape(seq.shape[0], -1) != 0, axis=1).sum())
    if real_frames < MIN_VALID_FRAMES:
        print(f"  [SKIP] {Path(filepath).name}: {real_frames} valid frames after normalization < {MIN_VALID_FRAMES}")
        return None
...
def build_tensors(samples, max_frames, transform=None):
    N = len(samples)
    data   = np.zeros((N, max_frames, NUM_JOINTS * CHANNELS), dtype=np.float32)
    labels = []
    names  = []
    errors = 0
...
def make_version(out_dir: Path, train_s, val_s, max_frames, version_name, transform=None):
    ...
    tc = Counter(tl)
    vc = Counter(vl)
    print(f"    x_train  shape={td.shape}  not_fall={tc[0]}  fall={tc[1]}")
    print(f"    x_test   shape={vd.shape}  not_fall={vc[0]}  fall={vc[1]}")
