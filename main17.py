...
def get_parser():
    ...
    parser.add_argument(
        ...
        action=DictAction,
        default=dict(),
        help='the arguments of loss function')
...
class Processor():
    ...
    def load_model(self):
        ...
        print(self.model)
        loss_weight = self.arg.loss_args.get('weight', None)
        weight_tensor = torch.tensor(loss_weight, dtype=torch.float).cuda(output_device) if loss_weight else None
        self.loss = nn.CrossEntropyLoss(weight=weight_tensor).cuda(output_device)
        ...
        for ln in loader_name:
            ...
            for batch_idx, (data, label, index) in enumerate(process):
                ...
                with torch.no_grad():
                    ...
                    label_cpu = label.cpu()
                    label_vis = label_cpu.numpy()
                    if (target_class_0 in label) or (target_class_1 in label) or (target_class_2 in label) or (target_class_3 in label):
                        selected_indices_0 = np.where(label_vis == target_class_0)
                        selected_indices_1 = np.where(label_vis == target_class_1)
                        selected_indices_2 = np.where(label_vis == target_class_2)
                        selected_indices_3 = np.where(label_vis == target_class_3)
                        selected_features_0 = output[selected_indices_0]
                        selected_features_1 = output[selected_indices_1]
                        selected_features_2 = output[selected_indices_2]
                        selected_features_3 = output[selected_indices_3]
                        selected_labels_0 = label[selected_indices_0]
                        selected_labels_1 = label[selected_indices_1]
                        selected_labels_2 = label[selected_indices_2]
                        ...
                        selected_features.append(selected_features_3)

                        selected_labels.append(selected_labels_0)
                        selected_labels.append(selected_labels_1)
                        selected_labels.append(selected_labels_2)
            ...
            if selected_features and selected_labels:
                ...
                for label in np.unique(selected_labels):
                    plt.scatter(embedded_features[selected_labels == label, 0],
                                embedded_features[selected_labels == label, 1], c=colors[label], s=10)
                plt.xticks([])
                plt.yticks([])
                # plt.show()
                ...
                print("No data for the selected classes.")
